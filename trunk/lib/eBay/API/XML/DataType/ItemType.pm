...
use eBay::API::XML::DataType::ReturnPolicyType;
...
my @gaProperties = ( [ 'ApplicationData', 'xs:string', '', '', '' ]
	...
	, [ 'ReservePrice', 'ns:AmountType', ''
	     ,'eBay::API::XML::DataType::AmountType', '1' ]
	, [ 'ReturnPolicy', 'ns:ReturnPolicyType', ''
	     ,'eBay::API::XML::DataType::ReturnPolicyType', '1' ]
...
specify an additional site-wide attribute set for the item condition.<br>
<br>
Older applications can still use AttributeSetArray to specify a return policy
(which is defined in a site-wide attribute set for most sites).
However, as eBay may deprecate return policy attributes in 2009, we strongly
recommend that you begin using the new Item.ReturnPolicy node instead to specify
the seller's return policy.<br>
<br>
...
Also use attributes to specify the item condition.<br>
<br>
...
, URL: AddItem.html#Request.Item.ReturnPolicy
Title: (AddItem) Item.ReturnPolicy
...
specified for the item and the seller specified a dispatch time.<br>
<br>
<b>For the AddItem family of calls:</b> See HandlingTimeEnabled in
GetCategoryFeatures to determine which categories require this field.<br>
<br>
...
SeeLink: URL: http://developer.ebay.com/DevZone/XML/docs/WebHelp/index.htm?context=eBay_XML_API&topic=HandlingTime
Title: Handling Time and Estimated Delivery Time
, URL: GetCategoryFeatures.html#Response.SiteDefaults.HandlingTimeEnabled
Title: (GetCategoryFeatures) SiteDefaults.HandlingTimeEnabled
...
Item Condition, use AttributeSetArray instead. You can use
AttributeSetArray for the condition and ItemSpecifics for the
...
the item condition (as new), predefined return policy (see Item.ReturnPolicy),
...
<br><br>
Starting in October 2008, on the US site, most categories only allow electronic payments.
<br><br>
Starting in October 2008, use GetCategoryFeatures to determine the default payment methods for a site,
and use it to determine the payment methods allowed for a category.
<br><br>
...
<br>
&lt;PaymentMethods&gt;CashOnPickup&lt;/PaymentMethods&gt;<br>
...
API Guide for information about payment methods required and allowed.
</span><br>
<br>
...
=head2 setReturnPolicy()

Describes the seller's return policy. Most eBay categories require
the seller to clearly specify whether or not returns are accepted
(see ReturnsAcceptedOption). <br>
<br>
<span class="tablenote"><b>Note:</b>
Items that were listed prior to October 2008 might not specify
a return policy.
</span><br>
<br>
<b>For the AddItem family of calls:</b> See ReturnPolicyEnabled in
GetCategoryFeatures to determine which categories require this field.
See ReturnPolicyDetails in GeteBayDetails to determine which ReturnPolicy fields
can be used.<br>
<br>
<br><b>For ReviseItem only:</b>
If the listing has bids or sales and it ends within 12 hours, you can't change
the return policy details.
If the listing is a GTC listing that has sales or ends within 12 hours
(one or the other, but not both), you can add
a return policy to the GTC listing (but you can't change 
return policy details if already present).
If the listing has no bids or sales and more than 12 hours remain before
the listing ends, you can add or change the return policy.
When you revise your return policy, you only need to specify the fields
you want to add or change. You don't need to specify all the other ReturnPolicy fields again.
The other fields will retain their existing settings.<br>
<br>
<b>For the GetItem family of calls:</b> Only returned if the site you
sent the request to supports the seller's return policy. Typically, the return policy
details are only returned when the request is sent to the listing site.

SeeLink: URL: GetCategoryFeatures.html#Response.Category.ReturnPolicyEnabled
For: categories that require a return policy
Title: (GetCategoryFeatures) Category.ReturnPolicyEnabled
, URL: GeteBayDetails.html#Response.ReturnPolicyDetails
For: return policy fields that each site reports
Title: (GeteBayDetails) ReturnPolicyDetails
...
#    Argument: 'ns:ReturnPolicyType'

=cut

sub setReturnPolicy {
  my $self = shift;
  $self->{'ReturnPolicy'} = shift
}

=head2 getReturnPolicy()
...
#    Returns: 'ns:ReturnPolicyType'

=cut

sub getReturnPolicy {
  my $self = shift;
  return $self->_getDataTypeInstance( 'ReturnPolicy'
